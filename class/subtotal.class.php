<?php
	...
	***************************************************/
	...
	/***************************************************
	* \file		../subtotal/class/subtotal.class.php
	* \ingroup	InfraS
	* \brief	Class to manage SubTotal module
	***************************************************/
	...
	class TSubtotal
	{
		...
		* 
		* @param	CommonObject $object Document on which we want to add a subtotal line
		* @param	string       $label	 Label of line
		* @param	int          $qty	 Quantity to put on line (used to determine the level of the title or subtotal line, for example qty 1 for a title of level 1, qty 2 for a title of level 2, etc... and inversely for subtotal line with qty 99 for a subtotal of level 1, qty 98 for a subtotal of level 2, etc...)
		...
		*/
		static function addSubTotalLine(&$object, $label, $qty, $rang=-1, $desc = '')	// InfraS change
		{
			...
			if ($object->element=='facture') {
				$res	=  $object->addline($desc, 0, $qty, 0, 0, 0, 0, 0, '', '', 0, 0, 0, 'HT', 0, 9, $rang, TSubtotal::$module_number, '', 0, 0, null, 0, $label);
			} elseif ($object->element=='invoice_supplier') {
				$object->special_code	= TSubtotal::$module_number;
				$res					= $object->addline($label, 0, 0, 0, 0, $qty, 0, 0, 0, 0, 0, 0, 'HT', 9, $rang, false, array(), null, 0, 0, '', TSubtotal::$module_number);
		...
		*
		* @param	CommonObject	$object				Document on which we want to add missing subtotal lines
		...
		* @param int          $level	Level of title line (between 1 and 4 for example, but you can use the level you want just to be able to determine the hierarchy between titles and subtotals)
		* @param int          $rang		Rang where to add line (if -1 add at the end of document)
		* @param string       $desc		Description of line (only used for facture and propal for the moment, not used for supplier invoice and supplier propal)
		* @return int
		*/
		...
		/**
		* @param	CommonObject	$object	Document on which we want to add a subtotal line
		* @param	string			$label	Label of subtotal line
		* @param	int				$level	Level of subtotal line (between 1 and 4 for example, but you can use the level you want just to be able to determine the hierarchy between titles and subtotals, for example if you have a title of level 1 then you can put a subtotal of level 1 with this method, if you have a title of level 2 then you can put a subtotal of level 2 with this method, etc...)
		* @param	int				$rang	Rang where to add line (if -1 add at the end of document)
		* @return	int
		*/
		...
		*/
		...
		/**
		* @param	CommonObjectLine	$line	Line object we want to know if it's a subtotal line and get the level of subtotal if it's the case (level is determined by qty field, for example qty 90 for a subtotal of level 1, qty 91 for a subtotal of level 2, etc...)
		* @param	int					$level	Level of subtotal line to check (if -1 just check if it's a subtotal line without checking level)
		* @return	bool				
		*/
		...
		* @param	bool         $withBlockLine	Whether to duplicate only the line with lineid or also the block line (title and subtotal) associated with this line
		* @return	int
		*/
		...
		* @param bool         $withBlockLine Whether to include the block line (title and subtotal) associated with the line
		...
		* @return	int							0 = $level <= 9 for title, 90 < $level <= 99 for subtotal, -1 for other lines
		*/
		...
		*/
		public static function addRecapPage(&$parameters, &$origin_pdf, $fromInfraS = 0)	// InfraS change
		{
			...
			if ($object->element == 'propal') {
				...
				setEventMessage($langs->trans('SubTotalWarningRecapObjectElementUnknown', $object->element), 'warnings');
				return -1;
			}
