<?php
	...
	**************************************************/
	...
	/**************************************************
	* \file			subtotal/class/actions_subtotal.class.php
	* \ingroup		subtotal
	* \brief		Hook actions for Subtotal module
	*************************************************/
	...
	dol_include_once('/subtotal/backport/v19/core/class/commonhookactions.class.php');
	...
	*/
	class ActionsSubtotal extends \subtotal\RetroCompatCommonHookActions
	{
		...
		protected $subtotal_level_cur = 0;					// @var int Subtotal current level
		protected $subtotal_show_qty_by_default = false;	// @var bool Show subtotal qty by default
		protected $subtotal_sum_qty_enabled = false;		// @var bool Determine if sum on subtotal qty is enabled
		protected $tfieldKeepWithNcCache = null;			// @var null|array cache local de SUBTOTAL_TFIELD_TO_KEEP_WITH_NC
		/**
		* Constructor
		*
		...
		**/
		protected function getCachedParentTitle(&$object, $rang)
		{
			...
			if (!isset($object->context['subtotalCache']) || !is_array($object->context['subtotalCache'])) {
				$object->context['subtotalCache']	= array();
			}
			if (!isset($object->context['subtotalCache']['parentTitleByRang']) || !is_array($object->context['subtotalCache']['parentTitleByRang'])) {
				$object->context['subtotalCache']['parentTitleByRang']	= array();
			}
			if (array_key_exists($rang, $object->context['subtotalCache']['parentTitleByRang'])) {
				return $object->context['subtotalCache']['parentTitleByRang'][$rang];
			}
			// Si le cache a été pré-chauffé (warmPDFSubtotalCache), l'absence de clé => pas de parent
			if (!empty($object->context['subtotalCache']['warmed'])) {
				$object->context['subtotalCache']['parentTitleByRang'][$rang]	= false;
				return false;
			}
			$res	= TSubtotal::getParentTitleOfLine($object, $rang, 0);
			$object->context['subtotalCache']['parentTitleByRang'][$rang]	= $res;
			return $res;
		}

		/**
		*	Cache de la chaîne complète des titres englobants d'une ligne.
		*	Reproduit TSubtotal::getAllTitleFromLine mais en O(1) après pré-chauffage.
		...
		**/
		protected function getCachedAllTitleFromLine(&$object, &$line)
		{
			...
			if (isset($object->context['subtotalCache']['allTitleChainByRang'][$line->rang])) {
				return $object->context['subtotalCache']['allTitleChainByRang'][$line->rang];
			}
			if (!empty($object->context['subtotalCache']['warmed'])) {
				// Cache chauffé mais rang absent => chaîne vide
				return array();
			}
			return TSubtotal::getAllTitleFromLine($line);
		}
		...
		*	Utilise une pile de titres ouverts : title => push, subtotal => pop (sémantique alignée sur getParentTitleOfLine avec $lvl=0).
		*
		*	@param	CommonObject	$object	Document
		*	@return	void
		**/
		protected function warmPDFSubtotalCache(&$object)
		{
			...
			$object->context['subtotalCache']['parentTitleByRang']		= $parentByRang;
			$object->context['subtotalCache']['allTitleChainByRang']	= $chainByRang;
			$object->context['subtotalCache']['warmed']					= true;
		}
		...
		**/
		protected function getCachedTitleHasTotal(&$object, &$title_line, $strict_mode = false)
		{
			...
			if (!isset($object->context['subtotalCache']['titleHasTotalByKey']) || !is_array($object->context['subtotalCache']['titleHasTotalByKey'])) {
				$object->context['subtotalCache']['titleHasTotalByKey']	= array();
			}
			$key	= $title_line->rang.'_'.($strict_mode ? '1' : '0');
			if (array_key_exists($key, $object->context['subtotalCache']['titleHasTotalByKey'])) {
				return $object->context['subtotalCache']['titleHasTotalByKey'][$key];
			}
			$res	= TSubtotal::titleHasTotalLine($object, $title_line, $strict_mode, false);
			$object->context['subtotalCache']['titleHasTotalByKey'][$key]	= $res;
			return $res;
		}

		/**
		*	Cache local du tableau SUBTOTAL_TFIELD_TO_KEEP_WITH_NC (évite l'explode à chaque ligne).
		*
		*	@return	array
		**/
		protected function getNcTfieldKeepList()
		{
			if ($this->tfieldKeepWithNcCache === null) {
				$raw							= getDolGlobalString('SUBTOTAL_TFIELD_TO_KEEP_WITH_NC', '');
		...
		*/
		public function printFieldListSelect($parameters, &$object, &$action, $hookmanager)
		{
			...
			if (in_array('consumptionthirdparty', $contexts) && in_array($type_element, array('propal', 'order', 'invoice', 'supplier_order', 'supplier_invoice', 'supplier_proposal'))) {
				...
				$where	.= ' AND (d.special_code != '.$mod_num.' OR d.product_type != 9 OR d.qty > 9)';		// Not a title (can't use TSubtotal class methods in sql)
				$where	.= ' AND (d.special_code != '.$mod_num.' OR d.product_type != 9 OR d.qty < 90)';	// Not a subtotal (can't use TSubtotal class methods in sql)
				$where	.= ' AND (d.special_code != '.$mod_num.' OR d.product_type != 9 OR d.qty != 50)';	// Not a free line text (can't use TSubtotal class methods in sql)
			}
			return 0;
		}
		...
		*/
		public function editDictionaryFieldlist($parameters, &$object, &$action, $hookmanager)
		{
			...
			if ($parameters['tabname'] == $this->db->prefix().'c_subtotal_free_text') {
		...
		*/
		public function formObjectOptions($parameters, &$object, &$action, $hookmanager)
		{
			...
			if (in_array('ordercard', $contexts) || in_array('ordersuppliercard', $contexts) || in_array('propalcard', $contexts) || in_array('supplier_proposalcard', $contexts) || in_array('invoicecard', $contexts) || in_array('invoicesuppliercard', $contexts) || in_array('invoicereccard', $contexts) || in_array('expeditioncard', $contexts)) {
				...
				if ($object->statut == 0 && $createRight) {
					...
					if (in_array($action, array('add_title_line', 'add_total_line', 'add_subtitle_line', 'add_subtotal_line', 'add_free_text'))) {
						...
						if ($action == 'add_title_line') {
							$title	= !empty(GETPOST('title', 'restricthtml')) ? GETPOST('title', 'restricthtml') : $langs->trans('SubTotalTitle');
							$qty	= $level < 1 ? 1 : $level ;
							...
							$title	= !empty($title) ? $title : $langs->trans('SubTotalAddLineDescription');
							$qty	= 50;
						} elseif ($action == 'add_subtitle_line') {
							$title	= !empty(GETPOST('title', 'restricthtml')) ? GETPOST('title', 'restricthtml') : $langs->trans('SubTotalSubtitle');
							$qty	= 2;
						} elseif ($action == 'add_subtotal_line') {
							$title	= $langs->trans('SubSubTotal');
							$qty	= 98;
						} else {
							$title	= !empty(GETPOST('title', 'restricthtml')) ? GETPOST('title', 'restricthtml') : $langs->trans('SubTotal');
							$qty	= $level ? 100 - $level : 99;
						}
						if (getDolGlobalString('SUBTOTAL_AUTO_ADD_SUBTOTAL_ON_ADDING_NEW_TITLE') && $qty < 10) {
						...
						$formconfirm	= $form->formconfirm(dol_escape_htmltag($_SERVER["PHP_SELF"]).'?id='.$object->id.'&lineid='.$lineid, $langs->trans('SubTotalDeleteWithAllLines'), $langs->trans('SubTotalConfirmDeleteAllThisLines', $nbLines), 'confirm_delete_all_lines', '', 0, 1);
						print $formconfirm;
					}
					if (getDolGlobalString('SUBTOTAL_ALLOW_ADD_LINE_UNDER_TITLE')) {
						showSelectTitleToAdd($object);
					}
					if ($object->element != 'shipping' && $action != 'editline') {
						printNewFormat($object, $conf, $langs, $idvar);
					}
				}
				...
				billOrdersAddCheckBoxForTitleBlocks();
			}
			return 0;
		}
		...
		*/
		public function formBuilddocOptions($parameters, &$object)
		{
			...
			if (!getDolGlobalString('SUBTOTAL_HIDE_OPTIONS_BUILD_DOC') && (in_array('invoicecard', $contextArray) || in_array('invoicesuppliercard', $contextArray) || in_array('propalcard', $contextArray) || in_array('ordercard', $contextArray) || in_array('ordersuppliercard', $contextArray) || in_array('invoicereccard', $contextArray))) {
				...
				$hidesubdetails			= isset($_SESSION['subtotal_hidesubdetails_'.$parameters['modulepart']][$object->id]) ?  $_SESSION['subtotal_hidesubdetails_'.$parameters['modulepart']][$object->id] : 0;
				...
				$titleOptions			= $langs->trans('SubTotalOptions').'&nbsp;&nbsp;&nbsp;'.img_picto($langs->trans('Setup'), 'setup', 'style="vertical-align: bottom; height: 20px;"');
				...
				$out	.= '	<tr class = "subtotalfold" style = "'.$titleStyle.'"><td colspan = "6" align = "center" style = "font-size: 120%;">'.$titleOptions.'</td></tr>
								<tr class = "oddeven subtotalfoldable">
									<td colspan = "6" class = "right">
										<label for = "hideInnerLines">'.$langs->trans('SubTotalHideInnerLines').'</label>
				...
				if ((in_array('propalcard', $contextArray) && getDolGlobalString('SUBTOTAL_PROPAL_ADD_RECAP')) || (in_array('ordercard', $contextArray) && getDolGlobalString('SUBTOTAL_COMMANDE_ADD_RECAP')) || (in_array('ordersuppliercard', $contextArray) && getDolGlobalString('SUBTOTAL_COMMANDE_ADD_RECAP')) || (in_array('invoicecard', $contextArray) && getDolGlobalString('SUBTOTAL_INVOICE_ADD_RECAP')) || (in_array('invoicesuppliercard', $contextArray) && getDolGlobalString('SUBTOTAL_INVOICE_ADD_RECAP')) || (in_array('invoicereccard', $contextArray) && getDolGlobalString('SUBTOTAL_INVOICE_ADD_RECAP'))) {
					$out	.= '<tr class = "oddeven subtotalfoldable">
									<td colspan = "6" class = "right">
										<label for = "subtotal_add_recap">'.$langs->trans('SubTotalAddRecap').'</label>
										<input type = "checkbox" id = "subtotal_add_recap" name = "subtotal_add_recap" value = "1" '.(!empty(GETPOST('subtotal_add_recap', 'int')) ? 'checked = "checked"' : '').'/>
									</td>
								</tr>';
				}
			...
			if (in_array($action, array('builddoc', 'addline', 'confirm_valid', 'confirm_paiement'))) {
				...
				if ($line->product_type == 9 && $line->special_code == $this->module_number) {
					...
					if ($line->qty > 90) {
						...
						$TInfo									= get_totalLineFromObject($object, $line, 0, 1);
		...
		public function doActions($parameters, &$object, $action, $hookmanager)
		{
			...
			if (in_array($action, array('updateligne', 'updateline'))) {
				...
				if (in_array('invoicecard', $contextArray)
					...
					$TSessNames		= getSessionNames($contextArray);
					...
					foreach ($object->lines as &$line) {
						if ($line->product_type == 9 && $line->special_code == $this->module_number) {
							...
							$line->total_ht = get_totalLineFromObject($object, $line, false);
						}
					}
				}
				...
				if ($nbDuplicate > 0) {
					setEventMessage($langs->trans('SubTotalDuplicateSuccess', $nbDuplicate));
				} elseif ($nbDuplicate == 0) {
					setEventMessage($langs->trans('SubTotalDuplicateLineidNotFound'), 'warnings');
				} else {
					setEventMessage($langs->trans('SubTotalDuplicateError'), 'errors');
				}
				header('Location: ?id='.$object->id);
				exit;
		...
		*/
		public function pdfAddTotal(&$pdf, &$object, &$line, $label, $description, $posx, $posy, $w, $h)
		{
			...
			$bgStyle							= getPdfBackgroundStyle($pdf, 'SUBTOTAL_SUBTOTAL_BACKGROUND_COLOR', 'SUBTOTAL_BACKGROUND_CELL_HEIGHT_OFFSET', 'SUBTOTAL_BACKGROUND_CELL_POS_Y_OFFSET');
			...
			$style				= getDolGlobalString('SUBTOTAL_SUBTOTAL_STYLE') ? getDolGlobalString('SUBTOTAL_SUBTOTAL_STYLE') : 'B';
			$pdf->SetFont('', $style, 9);
			$curentCellPaddinds = $pdf->getCellPaddings();	// save curent cell padding
			$pdf->setCellPaddings($curentCellPaddinds['L'], $subtotalDefaultTopPadding, $curentCellPaddinds['R'], $subtotalDefaultBottomPadding);	// set cell padding with column content definition for old PDF compatibility
			...
			if (!$hidePriceOnSubtotalLines) {
				...
				if ($total_to_print !== '') {
					if (GETPOST('hideInnerLines', 'int')) {
						...
						$TInfo			= get_totalLineFromObject($object, $line, false, 1);
						$TTotal_tva		= $TInfo[3];
		...
		*/
		public function pdfAddTitle(&$pdf, &$object, &$line, $label, $description, $posx, $posy, $w, $h)
		{
			...
			$fillDescBloc				= false;
			$bgStyle					= getPdfBackgroundStyle($pdf, 'SUBTOTAL_TITLE_BACKGROUND_COLOR', 'SUBTOTAL_TITLE_BACKGROUND_CELL_HEIGHT_OFFSET', 'SUBTOTAL_TITLE_BACKGROUND_CELL_POS_Y_OFFSET');
			$fillBackground				= $bgStyle['fill'];
			$backgroundColor			= $bgStyle['color'];
			$backgroundCellHeightOffset	= $bgStyle['heightOffset'];
			$backgroundCellPosYOffset	= $bgStyle['posYOffset'];
			...
			$size_title = 9;
			if (getDolGlobalString('SUBTOTAL_TITLE_SIZE')) {
				$size_title = getDolGlobalString('SUBTOTAL_TITLE_SIZE');
			}
			if ($hideInnerLines) {
				if ($line->qty == 1) {
			...
			if ($this->isModSubtotalLine($parameters, $object)) {
				...
				if (!empty($parameters['infrasplus'])) {
					...
					if (empty($hidePriceOnSubtotalLines)) {
						...
						if (getDolGlobalInt('SUBTOTAL_MANAGE_COMPRIS_NONCOMPRIS')) {
						...
						if($total_to_print !== '') {
							if (GETPOST('hideInnerLines', 'int')) {
								...
								$TInfo							= get_totalLineFromObject($object, $object->lines[$i], false, 1);
								$TTotal_tva						= $TInfo[3];
		...
		*/
		public function pdf_getlineremisepercent($parameters = array(), &$object, &$action = '')
		{
			...
			if ($this->isModSubtotalLine($parameters, $object)) {
				...
				return 1;
			} elseif (!empty($hideprices) || !empty($hidesubdetails) || (getDolGlobalString('SUBTOTAL_MANAGE_COMPRIS_NONCOMPRIS') && (!empty($object->lines[$i]->array_options['options_subtotal_nc']) || TSubtotal::hasNcTitle($object->lines[$i])) )) {
		...
		*/
		public function pdf_getlineupwithtax($parameters = array(), &$object, &$action = '')
		{
			...
			if (!empty($hideprices) || !empty($hidesubdetails) || (getDolGlobalString('SUBTOTAL_MANAGE_COMPRIS_NONCOMPRIS') && (!empty($object->lines[$i]->array_options['options_subtotal_nc']) || TSubtotal::hasNcTitle($object->lines[$i])))) {
				if (!empty($hideprices) || !in_array(__FUNCTION__, $this->getNcTfieldKeepList())) {
					$this->resprints = ' ';
					return 1;
				}
		...
		*/
		public function beforePDFCreation($parameters = array(), &$object, &$action = '')
		{
			...
			// Invalidation + pré-chauffage du cache subtotal (évite O(n²) dans les boucles PDF ci-dessous)
			if (!isset($object->context) || !is_array($object->context)) {
				$object->context	= array();
			}
			$object->context['subtotalCache']	= array();
			$this->warmPDFSubtotalCache($object);
			$TContext	= explode(':', $parameters['context']);
			if (in_array('pdfgeneration', $TContext)) {
				...
				setDocTVA($pdf, $object);
				addNumerotation($object);
				...
				if (!empty($hideInnerLines)) { // si c une ligne de titre
					...
					$nbtva = count($TTvas);
					if(!empty($nbtva) && !empty($hideInnerLines) && getDolGlobalString('SUBTOTAL_REPLACE_WITH_VAT_IF_HIDE_INNERLINES')) {
						foreach ($TTvas as $tx => $val) {
							$l					= clone $line;
							$l->product_type	= 1;
							$l->special_code	= '';
		...
		public function pdf_writelinedesc($parameters = array(), &$object, &$action = '')
		{
			...
			if ($this->isModSubtotalLine($parameters, $object) ) {
				...
				if(!empty($hideprices) || !empty($hidesubdetails)) {
					// Ne réinitialiser fk_parent_line qu'une seule fois par génération PDF
					if (empty($object->context['subtotalCache']['fkParentLineReset'])) {
						foreach ($object->lines as &$line) {
						...
						if (!isset($object->context['subtotalCache'])) {
							$object->context['subtotalCache'] = array();
						}
						$object->context['subtotalCache']['fkParentLineReset'] = true;
					}
				}
				$line = &$object->lines[$i];
				...
				if ($line->special_code!=$this->module_number || $line->product_type!=9) {
					if ($object->statut == 0  && $createRight && getDolGlobalString('SUBTOTAL_ALLOW_DUPLICATE_LINE') && $object->element !== 'invoice_supplier') {
						...
						if (($object->element != 'shipping' && $object->element != 'delivery')&& !(TSubtotal::isModSubtotalLine($line)) && ( $line->fk_prev_id === null ) && !($action == "editline" && GETPOST('lineid', 'int') == $line->id)) {
							echo '<a name="duplicate-'.$line->id.'" href="'.$_SERVER['PHP_SELF'].'?'.$idvar.'='.$object->id.'&action=duplicate&lineid='.$line->id.'&token='.$newToken.'"><i class="'.getDolGlobalString('MAIN_FONTAWESOME_ICON_STYLE').' fa-clone" aria-hidden="true"></i></a>';
							?>
								<script type="text/javascript">
									$(document).ready(function() {
					...
					$data	= getHtmlData($parameters, $object, $action, $hookmanager);
					...
					if (!empty(getDolGlobalString('SUBTOTAL_USE_NEW_FORMAT'))) {
						...
						if ($line->qty <= 99 && $line->qty >= 91) {
							$subtotalBackgroundColor = getDolGlobalString('SUBTOTAL_SUBTOTAL_BACKGROUND_COLOR', '#adadcf');
							...
							$titleBackgroundColor = getDolGlobalString('SUBTOTAL_TITLE_BACKGROUND_COLOR', '#adadcf');
					...
					<?php } ?>
						...
						if (TSubtotal::isSubtotal($line)) {
							/* Total */
							$TSubtotalDatas				= get_totalLineFromObject($object, $line, false, 1);
							$total_line					= $TSubtotalDatas[0];
							$multicurrency_total_line	= $TSubtotalDatas[6];
					...
					if ($action == 'editline' && GETPOST('lineid', 'int') == $line->id && TSubtotal::isModSubtotalLine($line)) {
						include dol_buildpath('/subtotal/core/tpl/subtotalline_edit.tpl.php', 0);
					} else {
						include dol_buildpath('/subtotal/core/tpl/subtotalline_view.tpl.php', 0);
					}
					?>
					<?php
					if ($line->qty>90) {
							...
							if ($action == 'editline' && GETPOST('lineid', 'int') == $line->id && TSubtotal::isModSubtotalLine($line) ) {
								...
								if ($object->statut == 0  && $createRight && getDolGlobalString('SUBTOTAL_ALLOW_DUPLICATE_BLOCK') && $object->element !== 'invoice_supplier') {
									...
									if (TSubtotal::isTitle($line) && ( $line->fk_prev_id === null )) {
										print '	<a class="subtotal-line-action-btn" title="'.$langs->trans('SubTotalCloneLSubtotalBlock').'" href="'.$_SERVER['PHP_SELF'].'?'.$idvar.'='.$object->id.'&action=duplicate&lineid='.$line->id.'&token='.$newToken.'" >
								...
								if ($object->statut == 0  && $createRight && getDolGlobalString('SUBTOTAL_ALLOW_EDIT_BLOCK')) {
									print '		<a class="subtotal-line-action-btn"  href="'.$_SERVER['PHP_SELF'].'?'.$idvar.'='.$object->id.'&action=editline&token='.$newToken.'&lineid='.$line->id.'#row-'.$line->id.'">'.img_edit().'</a>';
								}
							}
						}
						?>
							...
							if ($action != 'editline' && $action != 'selectlines') {
								if ($object->statut == 0  && $createRight && !empty(getDolGlobalString('SUBTOTAL_ALLOW_REMOVE_BLOCK'))) {
									...
									if (!isset($line->fk_prev_id) || $line->fk_prev_id === null) {
										print '	<a class="subtotal-line-action-btn"  href="'.$_SERVER['PHP_SELF'].'?'.$idvar.'='.$object->id.'&action=ask_deleteline&lineid='.$line->id.'&token='.$newToken.'">'.img_delete().'</a>';
									}
									if (TSubtotal::isTitle($line) && (!isset($line->fk_prev_id) || (isset($line->fk_prev_id) && ($line->fk_prev_id === null))) ) {
										$img_delete		= img_delete($langs->trans('SubTotalDeleteWithAllLines'), ' style="color:#be3535 !important;" class="pictodelete pictodeleteallline"');
										print '	<a class="subtotal-line-action-btn"  href="'.$_SERVER['PHP_SELF'].'?'.$idvar.'='.$object->id.'&action=ask_deleteallline&lineid='.$line->id.'&token='.$newToken.'">'.$img_delete.'</a>';
									}
								}
							}
						?>
					...
					if ($object->statut == 0  && $createRight && !empty(getDolGlobalString('SUBTOTAL_MANAGE_COMPRIS_NONCOMPRIS')) && TSubtotal::isTitle($line) && $action != 'editline' && $action != 'selectlines') {
						print '	<td class="subtotal_nc">
									<input id="subtotal_nc-'.$line->id.'" class="subtotal_nc_chkbx" data-lineid="'.$line->id.'" type="checkbox" name="subtotal_nc" value="1" '.(!empty($line->array_options['options_subtotal_nc']) ? 'checked="checked"' : '').' />
								</td>';
					}
							...
							<script>
								$(document).ready(function () {
									...
									if (!$isExtraSelected) {
										...
										echo 'var trad = "'.$langs->trans('SubTotalShowExtrafields').'";';
										echo 'var extra = 0;';
									} else {
										echo 'all_tr_extrafields.show();';
										echo 'var trad = "'.$langs->trans('SubTotalHideExtrafields').'";';
										echo 'var extra = 1;';
									}
									?>
									$("div .subtotal_underline").append(
										...
										+ '<input type="hidden" name="showBlockExtrafields" id="showBlockExtrafields" value="' + extra + '" />');
											$(document).on('click', "#printBlocExtrafields", function() {
												...
												if(val == '0') {
													btnShowBlock.val('1');
													$("#printBlocExtrafields").html("<?php print $langs->trans('SubTotalHideExtrafields'); ?>");
													$(all_tr_extrafields).show();
												} else {
													btnShowBlock.val('0');
													$("#printBlocExtrafields").html("<?php print $langs->trans('SubTotalShowExtrafields'); ?>");
					...
					<td style="<?php TSubtotal::isFreeText($line) ? '' : 'font-weight:bold;'; ?>  <?php echo ($line->qty > 90) ? 'text-align:right' : '' ?> "><?php
						...
						if (empty($line->label)) {
							if ($line->qty >= 91 && $line->qty <= 99 && getDolGlobalInt('SUBTOTAL_CONCAT_TITLE_LABEL_IN_SUBTOTAL_LABEL')) {
								print  $line->description.' '.getTitle($object, $line);
							} else {
					...
					if ($object->statut == 0 && !getDolGlobalString('SUBTOTAL_ALLOW_REMOVE_BLOCK')) {
						$colspan++;
					}
					if ($object->element == 'delivery') {
						$colspan = 2;
						...
						<?php
							if ($object->element == 'shipping' && $object->statut == 0 && getDolGlobalString('SUBTOTAL_ALLOW_REMOVE_BLOCK')) {
								...
								if ($line->fk_prev_id === null) {
									print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=deleteline&amp;lineid='.$lineid.'&token='.$newToken.'">'.img_delete().'</a>';
								}
								if (TSubtotal::isTitle($line) && ($line->fk_prev_id === null) ) {
									$img_delete	 = img_delete($langs->trans('SubTotalDeleteWithAllLines'), ' style="color:#be3535 !important;" class="pictodelete pictodeleteallline"');
									print '<a href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;action=ask_deleteallline&amp;lineid='.$lineid.'&token='.$newToken.'">'.$img_delete.'</a>';
								}
							print '	</td>';
						}
						print "</tr>\r\n";
		...
		public function printOriginObjectSubLine($parameters, &$object, &$action, $hookmanager)
		{
			...
			if (in_array('ordercard', $contexts) || in_array('invoicecard', $contexts) || in_array('ordersuppliercard', $contexts) || in_array('invoicesuppliercard', $contexts)) {
				...
				if (TSubtotal::isModSubtotalLine($line)) {
					...
					if (empty($line->label)) {
						if ($line->qty >= 91 && $line->qty <= 99 && getDolGlobalInt('SUBTOTAL_CONCAT_TITLE_LABEL_IN_SUBTOTAL_LABEL')) {
							$object->tpl["sublabel"].=  $line->description.' '.getTitle($object, $line);
						} else {
					...
					if ($line->qty>90) {
						$total						= get_totalLineFromObject($object, $line, false);
		...
		*/
		public function addMoreActionsButtons($parameters, &$object, &$action, $hookmanager) {
			...
			$jsConfig	= array('langs'						=> array('SubtotalSummaryTitle' => $langs->trans('SubTotalQuickSummary')),
			...
			if (!getDolGlobalString('SUBTOTAL_DISABLE_SUMMARY')) {
				...
				print '<link rel="stylesheet" type="text/css" href="'.dol_buildpath('subtotal/css/summary-menu.css.php', 1).'">';
		...
		*/
		public function afterPDFCreation($parameters, &$pdf, &$action, $hookmanager)
		{
			...
			if ((getDolGlobalString('SUBTOTAL_PROPAL_ADD_RECAP') && $object->element == 'propal') || (getDolGlobalString('SUBTOTAL_COMMANDE_ADD_RECAP') && $object->element == 'commande') || (getDolGlobalString('SUBTOTAL_INVOICE_ADD_RECAP') && $object->element == 'facture')) {
				if (GETPOST('subtotal_add_recap', 'int') && empty($parameters['fromInfraS'])) {
		...
		*/
		public function printCommonFooter(&$parameters, &$objectHook, &$action, $hookmanager)
		{
			...
			if (in_array('invoicecard', $contextArray)
				...
				$object->fetch($id);
				$TLines			= TSubtotal::getAllTitleFromDocument($object);	//On récupère tous les titres sous-total
				...
				$jsConf	= array('linesToHide'			=> $TBlocksToHide,
								...
								'langs'					=> array('Subtotal_HideAll'		=> $langs->transnoentities('Subtotal_HideAll'),
