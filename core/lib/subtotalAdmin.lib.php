<?php
	...
	************************************************/
	...
	/************************************************
	* 	\file		../subtotal/core/lib/subtotalAdmin.lib.php
	* 	\ingroup	InfraS
	* 	\brief		Admin functions used by SubTotal module
	************************************************/
	...
	**/
	function subtotal_admin_prepare_head ()
	{
		global $langs, $conf, $user;

		$h		= 0;
		$head	= array();
		if (!empty($user->admin) || !empty($user->hasRight('subtotal', 'paramSubTotal'))) {
			$head[$h][0]	= dol_buildpath('/subtotal/admin/subtotal_setup.php', 1);
			$head[$h][1]	= $langs->trans('Parameters');
			$head[$h][2]	= 'subtotalsetup';
			$h++;
		}
		complete_head_from_modules($conf, $langs, null, $head, $h, 'subtotal_admin');
		$head[$h][0]	= dol_buildpath('/subtotal/admin/subtotal_about.php', 1);
		$head[$h][1]	= $langs->trans('About');
		$head[$h][2]	= 'about';
		$h++;
		$head[$h][0]	= dol_buildpath('/subtotal/admin/subtotal_changelog.php', 1);
		$head[$h][1]	= $langs->trans('SubTotalParamChangelog');
		$head[$h][2]	= 'changelog';
		complete_head_from_modules($conf, $langs, null, $head, $h, 'subtotal_admin', 'remove');
		return $head;
	}
	...
	*
	**/
	function subtotal_test_php_ext()
	{
		...
		if (extension_loaded('xml')) {
			dolibarr_set_const($db, 'INFRAS_PHP_EXT_XML',	1, 'chaine', 0, 'SubTotal module', $conf->entity);
		} else {
			dolibarr_set_const($db, 'INFRAS_PHP_EXT_XML',	-1, 'chaine', 0, 'SubTotal module', $conf->entity);
			setEventMessages('<span class = "subtotalcaution">'.$langs->trans('SubTotalCautionMess').'</span>'.$langs->trans('InfraSXMLextError'), array(), 'warnings');
		}
	}
	...
	*									[6] PHP max version
	**/
	function subtotal_getLocalVersionMinDoli($appliname)
	{
		global $langs;

		$currentversion	= array();
		$sxe			= subtotal_getChangelogFile($appliname);
		if (is_object($sxe))	{
			...
			$currentversion[0]	= '<span class="subtotalcaution"><b>'.$langs->trans('SubTotalChangelogXMLError').'</b></span>';
			$currentversion[1]	= $langs->trans('SubTotalnoMinDolVersion');
			$currentversion[2]	= -1;
			$currentversion[3]	= $langs->trans('SubTotalChangelogXMLError');
			$currentversion[4]	= $langs->trans('SubTotalnoMaxDolVersion');
			$currentversion[5]	= $langs->trans('SubTotalnoMinPHPVersion');
			$currentversion[6]	= $langs->trans('SubTotalnoMaxPHPVersion');
			foreach (libxml_get_errors() as $error) {
				$currentversion[3]	.= $error->message;
				dol_syslog('subtotal.Lib::subtotal_getLocalVersionMinDoli error->message = '.$error->message);
			}
		}
	...
	**/
	function subtotal_getChangelogFile($appliname, $from = '')
	{
		...
		if (is_file($file)) {
			...
			dol_syslog('subtotalAdmin.Lib::subtotal_getChangelogFile appliname = '.$appliname.' from = '.$from.' context = '.$context.' changelog = '.($changelog ? 'Ok' : 'KO').' sxe = '.($sxe ? 'Ok' : 'KO'));
			return $sxe;
		} else {
			return false;
		}
	...
	**/
	function subtotal_dwnChangelog($appliname)
	{
	...
	**/
	function subtotal_bkup_module ($appliname)
	{
		global $db, $conf, $langs;
...
-- SubTotal version	'.$currentversion[0].'
...
';
			...
			$sql_const		.= ' WHERE name LIKE "SUBTOTAL\_%"';
			$sql_const		.= ' AND entity = '.((int) $conf->entity);
			$sql_const		.= ' ORDER BY name';
			fwrite($handle, subtotal_bkup_table ('const', $sql_const, $cols_const, $duplicate, 0, ''));
...
';
	...
	**/
	function subtotal_bkup_table ($table, $sql, $listeCols, $duplicate = array (), $truncate = 0, $add = '')
	{
		global $db;

		$sqlnewtable	= '';
		$result_sql		= $sql ? $db->query($sql) : '';
		dol_syslog('subtotal.Lib::subtotal_bkup_table sql = '.$sql);
		if ($result_sql) {
...
';
	...
	**/
	function subtotal_restore_module ($appliname)
	{
		global $conf;
		...
		if (is_resource($handle)) {
			...
			dol_syslog('subtotal.Lib::subtotal_restore_module appliname = '.$appliname.' filesql = '.$filesql.' moved = '.$moved.' result = '.$result.' delete = '.$delete);
			if ($result > 0) {
				return 1;
			}
		}
	...
	*	@return	string			value .
	**/
	function subtotal_return_bytes($val)
	{
		$val	= trim($val);
	...
	*	@return		void
	**/
	function subtotal_print_backup_restore()
	{
		global $langs, $conf;
		...
		print '		<tr>
						<td colspan = "2" class = "center subtotaltitleparam">
							<a href = "'.DOL_URL_ROOT.'/document.php?modulepart=subtotal&file=sql/update.'.$conf->entity.'">'.$langs->trans('SubTotalParamAction1').' <b><span>'.$langs->trans('modcomnameSubTotal').'</span></b> '.$langs->trans('SubTotalParamAction2').'</a>
						</td>
						<td align = "center"><button class = "butAction" type = "submit" value = "bkupParams" name = "action">'.$langs->trans('SubTotalParamBkup').'</button></td>
						<td align = "center"><button class = "butAction" type = "submit" value = "restoreParams" name = "action">'.$langs->trans('SubTotalParamRestore').'</button></td>
					</tr>';
	...
	*	@return	string
	**/
	function subtotal_load_title($titre, $morehtmlright = '', $picto = 'generic', $pictoisfullpath = 0, $id = '', $morecssontable = '', $morehtmlcenter = '')
	{
		$out	= '';
		if ($picto == 'setup')	{
			$picto	= 'generic';
		...
		if (!empty($picto)) {
			$out .= '			<td class = "nobordernopadding widthpictotitle valignmiddle col-picto">'.img_picto('', $picto, 'class = "valignmiddle subtotalwidthpictotitle pictotitle"', $pictoisfullpath).'</td>';
		}
		$out	.= '			<td class = "nobordernopadding valignmiddle col-title"><div class = "subtotalDivTitre  uppercase inline-block">'.$titre.'</div></td>';
	...
	*	@return		void
	**/
	function subtotal_print_colgroup($metas = array())
	{
		print '	<tr>';
		foreach ($metas as $values)	{
			print '<td class = "subtotalFinal subtotalnopadding"'.($values == '*' ? '' : ' width = "'.$values.'"').' style =" height: 1px;'.($values == '*' ? '' : ' max-width: '.$values.'; min-width: '.$values.'; width: '.$values.';').'">&nbsp;</td>';
		}
		print '	</tr>';
	}
	...
	*	@return		void
	**/
	function subtotal_print_liste_titre($metas = array())
	{
		global $langs;
	...
	*	@return		void
	**/
	function subtotal_print_btn_action($action, $desc = '', $cs1 = 3, $alignclass = 'center', $lbl = 'Modify', $noRowspan = false)
	{
		global $langs;

		print '	<tr>
					<td colspan = "'.$cs1.'" class = "'.$alignclass.'">'.$desc.'</td>
					<td'.(empty($noRowspan) ? ' rowspan = "0"' : '').' class = "center valigntop"><button class = "button subtotalwidth110" type = "submit" value = "update_'.$action.'" name = "action">'.$langs->trans($lbl).'</button></td>
				</tr>';
	}
	...
	*	@return		void
	**/
	function subtotal_print_hr($cs1 = 3)
	{
		print '	<tr><td colspan = "'.$cs1.'"><hr class = "subtotalHR"></td></tr>';
	}
	...
	*	@return		void
	**/
	function subtotal_print_subTitle($cs1 = 3, $subtitle = '')
	{
		global $langs;

		subtotal_print_hr($cs1);
		print '	<tr>
					<td colspan = "'.$cs1.'" class = "center"><span class = "subtotalSubtitleParam">'.$langs->trans($subtitle).'</span></td>
				</tr>';
	}
	...
	*	@return		void
	**/
	function subtotal_print_final($cs1 = 3)
	{
		print '	<tr><td colspan = "'.$cs1.'" class = "subtotalFinal">&nbsp;</td></tr>';
	}
	...
	*	@return		int
	**/
	function subtotal_print_input($confkey, $tag = 'on_off', $desc = '', $help = '', $metas = array(), $cs1 = '2', $cs2 = '1', $end = '', $num = 0)
	{
		...
		if ($tag == 'on_off') {
			...
			// default input
			$defaultMetas		= array('type' => 'text', 'class' => 'flat quatrevingtpercent subtotalnopadding subtotalfontsizeinherit', 'name' => $confkey, 'id' => $confkey, 'value' => $inputValue);
	...
	**/
	function subtotal_print_line_inputs($type = '', $desc = '', $metas = array(), $cs1 = 2, $w = 0, $end = '', $num = 0)
	{
		print '	<tr class = "oddeven">';
		if (!empty($num)) {
			print '	<td class = "center bold">'.$num.'</td>';
		...
		print '		<td colspan = "'.$cs1.'">
						<table class = "centpercent">
							<tr>
								<td rowspan = "2" class = "subtotalnoborder">'.$desc.'</td>';
		foreach ($metas[0] as $confkey => $value) {
			$confkey	= str_replace('_AUTO', '', $confkey);
			print '				<td class = "center subtotalnoborder" style = "max-width: '.$w.'px; min-width: '.$w.'px; width: '.$w.'px;">'.($type == 'tests' ? (getDolGlobalString($confkey, '') ? $value : '&nbsp;') : $value).'</td>';
		}
		print '				</tr>
							<tr>';
		foreach ($metas[1] as $confkey => $value) {
			print '				<td class = "center subtotalnoborder">';
	...
	**/
	function subtotal_getChangeLog($appliname, $version, $resVersion, $tblversions, $dwn = 0)
	{
		global $langs, $user;
		...
		$InputCarac				= 'class = "butAction subtotalnopadding subtotalwidth180 subtotalheight32" name = "readmore" type = "button"';
		$supportvalue			= '/******************************'.'<br/>';
		$supportvalue			.= ' * Module : '.$langs->trans('modcomnameSubTotal').'<br/>';
		...
		$ret					= '	<form id = "ticket" method = "POST" target = "_blank" action = "'.$supportURL.'">
										...
										<input name = category_code type = "hidden" value = "'.(strtoupper($langs->trans('modcomnameSubTotal'))).'" />
										<table class = "centpercent" style = "padding: 10; background: url('.$headerPath.'); background-size: cover;">
											<tr class = "subtotalheight75">
												<td colspan = "3" class = "center bold valignmiddle">
													<a href = "'.$urlWiki.'" target = "_blank">
														<span class = "subtotalcolor" style = "font-size: 24px;">'.$langs->trans('SubTotalParamPresent1').'<span class = "subtotalneuropolinfras"> InfraS</span>'.$langs->trans('SubTotalParamPresent2').'</span>
													</a>
												</td>
											</tr>
											<tr class = "subtotalheight50">
												<td rowspan = "3" class = "left bold valignbottom subtotalwidthtrentepercent subtotalslogan" style = "color: white; font-size: 16px;">
													<a href = "'.$urlInfraS.'" target = "_blank"><img class = "subtotalnoborder subtotalwidth220" src = "'.$logoPath.'"></a>
													<br/>&nbsp;&nbsp;'.$langs->trans('SubTotalParamSlogan').'
												</td>
												<td class = "center valignmiddle subtotalwidthtrentepercent">
													<a href = "'.$urlstore.'" target = "_blank"><input '.$InputCarac.' value = "'.$langs->trans('SubTotalParamLienModules').'" /></a>
													<button class = "butAction subtotalnopadding subtotalwidth180 subtotalheight32" type = "submit" >'.$langs->trans('SubTotalParamSupport').'</button>
												</td>
												<td rowspan = "3" class = "right bold valignbottom subtotalwidthtrentepercent subtotalslogan">
													<a href = "'.$urlDoli.'" target = "_blank"><img class = "subtotalnoborder subtotalwidth270" src = "'.$logoDolistorePath.'"></a>&nbsp;&nbsp;
													<br/>'.$langs->trans('SubTotalParamMoreModulesLink').'&nbsp;&nbsp;
												</td>
											</tr>
											<tr>
												<td class = "center valignbottom subtotalminwidth700imp">
													<img class = "subtotalnoborder subtotalwidth220 subtotalmargintop10imp" src="'.$preferedPartnerPath.'"/>
												</td>
											</tr>
											<tr>
												<td class = "center bold valignbottom subtotalminwidth700imp subtotalslogan">
													<div class = "subtotalmargintop10imp">'.$langs->trans('SubTotalParamPreferedPartner1').'<span class = "subtotalpuentedolibarr"> Dolibarr </span>'.$langs->trans('SubTotalParamPreferedPartner2').'</div>
												</td>
											</tr>
											<tr class = "subtotalheight25"><td colspan = "3">&nbsp;</td></tr>
										</table>
									</form>';

		$ret					.= load_fiche_titre($langs->trans("SubTotalParamHistoryUpdates"), '', $listUpD, 1);
		$sxe					= subtotal_getChangelogFile($appliname);
		$sxelast				= subtotal_getChangelogFile($appliname, 'dwn');
		...
		if (getDolGlobalString('INFRAS_SKIP_CHECKVERSION', ''))	{
			$dwnbutton	= $dwn ? $langs->trans("SubTotalParamSkipCheck") : '';
		} else {
			$dwnbutton	= $dwn ? '<button class = "button" style = "width: 190px; padding: 3px 0px;" type = "submit" value = "dwnChangelog" name = "action" title = "'.$langs->trans("SubTotalParamCheckNewVersionTitle").'">'.$langs->trans("SubTotalParamCheckNewVersion").'</button>' : '';
		}
		$ret	.= '		<form action = "'.dol_escape_htmltag($_SERVER['PHP_SELF']).'" method = "post" enctype = "multipart/form-data">
								<input type = "hidden" name = "token" value = "'.newToken().'">
								<table class = "subtotalnoborder centpercent" >
									<tr class = "liste_titre">
										<th class = "center width100">'.$langs->trans('SubTotalParamNumberVersion').'</th>
										<th class = "center width100">'.$langs->trans('SubTotalParamMonthVersion').'</th>
										<th class = left >'.$langs->trans('SubTotalParamChangesVersion').'</th>
		...
		if (is_object($sxe) && count($tblversionslast) > count($tblversions)) {	// il y a du nouveau
			for ($i = count($tblversionslast)-1; $i >= 0; $i--) {
				...
				dol_syslog('subtotal.Lib::subtotal_getChangeLog sxePath = '.$sxePath);
				if (empty($sxePath))	$color='bgcolor = orange';
				$lineversion			= $tblversionslast[$i]->change;
				$ret			.= '<tr class = "oddeven">
										<td class = "center valigntop '.(empty($sxePath) ? 'subtotalbgorange' : '').'">'.$tblversionslast[$i]->attributes()->Number.'</td>
										<td class = "center valigntop '.(empty($sxePath) ? 'subtotalbgorange' : '').'">'.$tblversionslast[$i]->attributes()->MonthVersion.'</td>
										<td class = "left valigntop subtotalnopaddingvert '.(empty($sxePath) ? 'subtotalbgorange' : '').'" colspan = "2">';
				foreach ($lineversion as $changeline) {
					if ($changeline->attributes()->type == 'fix') {
						$classcolor	= ' subtotalcaution';
					} elseif ($changeline->attributes()->type == 'add') {
						$classcolor	= ' subtotalgreen';
					} elseif ($changeline->attributes()->type == 'chg') {
						$classcolor	= ' subtotalblue';
					} else {
						$classcolor	= ' subtotalblack';
					}
					$ret	.= '			<table>
												<tr>
													<td class = "width50 subtotalchangelogbase'.$classcolor.'">'.$changeline->attributes()->type.'</td>
													<td class = "subtotalchangelogbase'.$classcolor.'">'.$changeline.'</td>
												</tr>
											</table>';
				}
				$ret	.= '			</td>
			...
			for ($i = count($tblversions)-1; $i >= 0; $i--) {
				...
				$ret			.= '<tr class = "oddeven">
										<td class = "center valigntop '.(empty($sxelastPath) ? 'subtotalbggreen subtotalblack' : '').'">'.$tblversions[$i]->attributes()->Number.'</td>
										<td class = "center valigntop '.(empty($sxelastPath) ? 'subtotalbggreen subtotalblack' : '').'">'.$tblversions[$i]->attributes()->MonthVersion.'</td>
										<td class = "left valigntop subtotalnopaddingvert '.(empty($sxelastPath) ? 'subtotalbggreen' : '').'" colspan = "2">';
				foreach ($lineversion as $changeline) {
					if ($changeline->attributes()->type == 'fix') {
						$classcolor	= ' subtotalcaution';
					} elseif ($changeline->attributes()->type == 'add') {
			...
			for ($i = count($tblversions)-1; $i >= 0; $i--) {
				...
				$ret			.= '<tr class = "oddeven">
										...
										<td class = "left valigntop subtotalnopaddingvert" colspan = "2">';
				foreach ($lineversion as $changeline) {
					if ($changeline->attributes()->type == 'fix') {
						$classcolor	= ' subtotalcaution';
					} elseif ($changeline->attributes()->type == 'add') {
	...
	* @return	string					HTML presentation
	**/
	function subtotal_getSupportInformation($currentversion)
	{
		global $db, $langs;

		$ret	= '<table class="subtotalnoborder" >
						<tr class="liste_titre">
							<th align = center width=200px>'.$langs->trans('SubTotalSupportInformation').'</th>
