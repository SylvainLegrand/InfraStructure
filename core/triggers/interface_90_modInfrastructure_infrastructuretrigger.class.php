	...
	class InterfaceInfrastructuretrigger extends DolibarrTriggers
	{
		...
		*/
		public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
		{
			...
			if (getDolGlobalString('INFRASTRUCTURE_ALLOW_ADD_LINE_UNDER_TITLE') && in_array($action, array('LINEPROPAL_INSERT', 'LINEORDER_INSERT', 'LINEBILL_INSERT'))) {
			...
			if (getDolGlobalString('INFRASTRUCTURE_MANAGE_COMPRIS_NONCOMPRIS') && in_array($action, array('LINEPROPAL_INSERT', 'LINEPROPAL_MODIFY', 'LINEORDER_INSERT', 'LINEORDER_MODIFY', 'LINEBILL_INSERT', 'LINEBILL_MODIFY', 'LINEBILL_SUPPLIER_CREATE', 'LINEBILL_SUPPLIER_MODIFY'))) {
		...
		private function OrdersToInvoiceBloc($object, $user, $action, $langs)
		{
			...
			if ($ret > 0 && !$infrastructure_bloc_already_add_st) {
				...
				if ($last_fk_commandedet === (int) $object->origin_id && !empty($current_fk_commande)) {
					...
					$rang += 2;
					TInfrastructure::addTotal($facture, $langs->trans('Infrastructure'), 1, $rang);
		...
		*/
		private function ComprisNonCompris($object, $user, $action, $langs)
		{
			...
			foreach ($TTitle as &$line) {
				if (!empty($line->array_options['options_infrastructure_nc'])) {
			...
			if (!empty($object->array_options['options_infrastructure_nc'])) {
				$object->total_ht = $object->total_tva = $object->total_ttc = $object->total_localtax1 = $object->total_localtax2 = $object->multicurrency_total_ht = $object->multicurrency_total_tva = $object->multicurrency_total_ttc = 0;
			...
			if (!empty($parent_element) && !empty($object->array_options['options_infrastructure_nc'])) {
				infrastructure_updateLineNC($parent_element, $object->{'fk_'.$parent_element}, $object->id, $object->array_options['options_infrastructure_nc'], 1);
			}
		}
		...
		*/
		...
		{
			$doli_action = GETPOST('action', 'aZ09');
			if (!getDolGlobalString('INFRASTRUCTURE_MANAGE_COMPRIS_NONCOMPRIS') || !in_array($doli_action, array('confirm_clone'))) {
				return;
			}
			dol_syslog("[INFRASTRUCTURE_MANAGE_COMPRIS_NONCOMPRIS] Trigger '".$this->name."' for action '".$action."' launched by ".__FILE__.". object=".$object->element." id=".$object->id);
			...
			foreach ($object->lines as &$line) {
				...
				if (!TInfrastructure::isModInfrastructureLine($line) && !empty($line->array_options['options_infrastructure_nc'])) {
					...
					if ($res > 0) {
						setEventMessage($langs->trans('infrastructure_update_nc_success'));
					}
				}
			}
			if (!empty($line)) {
