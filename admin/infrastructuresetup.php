<?php
	...
	*************************************************/
	...
	if (preg_match('/update_(.*)/', $action, $reg)) {
		$list				= ['Gen'	=> ['INFRASTRUCTURE_LIST_OF_EXTRAFIELDS_PROPALDET',	'INFRASTRUCTURE_LIST_OF_EXTRAFIELDS_COMMANDEDET',
													'INFRASTRUCTURE_LIST_OF_EXTRAFIELDS_FACTUREDET',
													'INFRASTRUCTURE_TEXT_FOR_TITLE_ORDERS_TO_INVOICE'
													],
									'aff'	=> ['INFRASTRUCTURE_DEFAULT_DISPLAY_QTY_FOR_TOTAL_ON_ELEMENTS',
		...
		$confkey			= $reg[1];
		$error				= 0;
		$arraysToImplode	= ['INFRASTRUCTURE_LIST_OF_EXTRAFIELDS_PROPALDET',
									'INFRASTRUCTURE_LIST_OF_EXTRAFIELDS_COMMANDEDET',
									'INFRASTRUCTURE_LIST_OF_EXTRAFIELDS_FACTUREDET',
									'INFRASTRUCTURE_DEFAULT_DISPLAY_QTY_FOR_TOTAL_ON_ELEMENTS',
									'INFRASTRUCTURE_DEFAULT_DISPLAY_QTY_FOR_TOTAL_ON_ELEMENTS_PDF'
									];
		foreach ($list[$confkey] as $constname) {
			if (in_array($constname, $arraysToImplode)) {
	...
	// init variables *******************************
	$colWith_OL			= ['pdf_getlineqty'				=> $langs->trans('Qty'),
	...
	$metas	= ['30px', '*', '90px', '156px', '120px'];
	infrastructure_print_colgroup($metas);
	$metas	= [[1, 2, 1, 1], 'NumberingShort', 'Description', $langs->trans('Status').' / '.$langs->trans('Value'), '&nbsp;'];
	...
	if (!empty($accessright)) {
		...
		$num	= 1;
		$num	= infrastructure_print_input('INFRASTRUCTURE_ALLOW_ADD_BLOCK', 'on_off', $langs->trans('InfrastructureAllowAddBlock'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_ALLOW_EDIT_BLOCK', 'on_off', $langs->trans('InfrastructureAllowEditBlock'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_ALLOW_REMOVE_BLOCK', 'on_off', $langs->trans('InfrastructureAllowRemoveBlock'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_ALLOW_DUPLICATE_BLOCK', 'on_off', $langs->trans('InfrastructureAllowDuplicateBlock'), '', [], 2, 1, '', $num);
		// num = 5
		$num	= infrastructure_print_input('INFRASTRUCTURE_ALLOW_DUPLICATE_LINE', 'on_off', $langs->trans('InfrastructureAllowDuplicateLine'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_ALLOW_ADD_LINE_UNDER_TITLE', 'on_off', $langs->trans('InfrastructureAllowAddLineUnderTitle'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_ADD_LINE_UNDER_TITLE_AT_END_BLOCK', 'on_off', $langs->trans('InfrastructureAddLineUnderTitleAtEndBlock'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_HIDE_FOLDERS_BY_DEFAULT', 'on_off', $langs->trans('InfrastructureHideFoldersByDefault'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_HIDE_OPTIONS_TITLE', 'on_off', $langs->trans('InfrastructureHideOptionsTitle'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_HIDE_OPTIONS_BREAK_PAGE_BEFORE', 'on_off', $langs->trans('InfrastructureHideOptionsBreakPageBefore'), '', [], 2, 1, '', $num);
		// num = 11
		if ($isV20p) {
			$num	= infrastructure_print_input('INFRASTRUCTURE_FORCE_EXPLODE_ACTION_BTN', 'on_off', $langs->trans('InfrastructureForceExplodeActionBtn'), 'InfrastructureForceExplodeActionBtnInfo', [], 2, 1, '', $num);
		} else {
			$num++;
		}
	...
	if (!empty($accessright)) {
		...
		$num	= 1;
		$num	= infrastructure_print_input('INFRASTRUCTURE_USE_NUMEROTATION', 'on_off', $langs->trans('InfrastructureUseNumerotation'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_PDF_TITLE_WITH_TOTAL', 'on_off', $langs->trans('InfrastructurePdfTitleWithTotal'), 'InfrastructurePdfTitleWithTotalInfo', [], 2, 1, '', $num);
		// num = 3
		$metas	= ['class' => 'flat infrastructurewidth270 infrastructurefontsizeinherit'];
		...
		$num	= infrastructure_print_input('INFRASTRUCTURE_PDF_TITLE_BACKGROUND_COLOR', 'color', $langs->trans('InfrastructurePdfTitleBackgroundcolor'), '', [], 2, 1, '', $num);
		$num	= infrastructure_print_input('INFRASTRUCTURE_PDF_TITLE_COLOR', 'color', $langs->trans('InfrastructurePdfTitleColor'), 'InfrastructurePdfTitleColorInfo', [], 2, 1, '', $num);
		// num = 9
		if (empty($titleWithTotal)) {
		...
		if (empty($titleWithTotal)) {
			$num	= infrastructure_print_input('INFRASTRUCTURE_SHOW_TVA_ON_TOTAL_LINES', 'on_off', $langs->trans('InfrastructureShowTvaOnTotalLines'), '', [], 2, 1, '', $num);
			if (getDolGlobalInt('INFRASTRUCTURE_SHOW_TVA_ON_TOTAL_LINES')) {
				$num	= infrastructure_print_input('INFRASTRUCTURE_LIMIT_TVA_ON_CONDENSED_BLOCS', 'on_off', $langs->trans('InfrastructureLimitTvaOnCondensedBlocs'), '', [], 2, 1, '', $num);
			} else {
				$num++;
			}
		} else {
