...
- Dernière version locale : `18.2.0` (2026-04)
...
- **Constantes prédéfinies** (dans `$this->const`) : 6 entrées
	...
	- `INFRASTRUCTURE_TOTAL_STYLE` (chaine, défaut `B`) — Style des sous-totaux (B=Gras)
...
- `INFRASTRUCTURE_TOTAL_STYLE` — style des sous-totaux — défaut `B`
- `INFRASTRUCTURE_TOTAL_BACKGROUND_COLOR` — couleur de fond utilisée pour les sous-totaux
- `INFRASTRUCTURE_TOTAL_COLOR` — couleur de texte utilisée pour les icônes de total
...
- `18.2.0` (2026-04) : Pliage des blocs titre — le bouton « plier sans toucher aux titres enfants » (`data-toggle-all-children="0"`) n'est plus rendu dans `core/tpl/infrastructureline_view.tpl.php` quand `INFRASTRUCTURE_BLOC_FOLD_MODE` n'est pas réglé sur `'keepTitle'`. En mode par défaut, ce bouton produisait exactement le même rendu visuel que le bouton « plier tout » (`data-toggle-all-children="1"`) puisque toutes les lignes filles, y compris sous-titres et sous-totaux, sont masquées indistinctement ; la présence des deux boutons identiques était une source de confusion utilisateur. Le bouton « plier tout » reste toujours affiché.
- `18.2.0` (2026-04) : Suppression de l'option INFRASTRUCTURE_USE_NEW_FORMAT : le format avancé (niveaux de titres et sous-totaux 1 à 9, icônes et coloration par niveau, sélecteur de niveau dans les boîtes de dialogue et l'édition de ligne) devient le comportement systématique du module. Nettoyage des branches conditionnelles dans actions_infrastructure.class.php, core/lib/infrastructure.lib.php et les templates infrastructureline_view.tpl.php / infrastructureline_edit.tpl.php ; suppression de la ligne d'admin dans infrastructuresetup.php ; suppression de la constante par défaut dans sql/data.sql et des clés de traduction InfrastructureUseNewFormat[Help] dans les 4 fichiers de langue.
- `18.2.0` (2026-04) : Reformulation des libellés des trois options du dropdown `INFRASTRUCTURE_BLOC_FOLD_MODE` dans les 4 fichiers de langue. Les libellés précédents ne décrivaient pas le comportement réel : `default` annonçait « cacher les lignes de titre et sous-total enfant » alors qu'il masque l'intégralité du contenu (y compris les lignes produit), et `keepTitle` / `hideAll` n'explicitaient pas la différence entre les modes. Les nouveaux libellés indiquent clairement ce qui est masqué (lignes produit uniquement vs tout le contenu) et le nombre de boutons effectifs.
- `18.2.0` (2026-04) : Affinage des libellés des trois options du dropdown `INFRASTRUCTURE_BLOC_FOLD_MODE`. La référence fr_FR a été revue (terme « lignes article » au lieu de « lignes produit » ; pour `keepTitle` distinction explicite « directement sous le titre » vs « y compris celles des titres imbriqués » ; pour `hideAll` second bouton décrit comme « masquer tout le contenu du bloc »). Les versions en_US, es_ES, it_IT ont été synchronisées sur la nouvelle référence fr_FR.
- `18.2.0` (2026-04) : Rendu de la marge sur les lignes sous-total (`INFRASTRUCTURE_DISPLAY_MARGIN_ON_TOTAL`) — refonte du layout dans `core/tpl/infrastructureline_infrastructure.tpl.php`. (1) suppression du libellé « Marge : » (la valeur seule est désormais affichée), (2) déplacement de la valeur dans la dernière colonne avant Total HT (la colonne Marge native du module margin — classes `margininfos right`), (3) le libellé « Sous-total : » passe à gauche de la marge. Cellule marge non rendue si module margin inactif ou `margins_hidden_by_module` positionné, pour éviter un décalage de colonnes.
- `18.2.0` (2026-04) : Option « Afficher la marge sur les sous-totaux » — **fix** d'un nom de constante incohérent qui empêchait l'option de fonctionner (le template lisait `DISPLAY_MARGIN_ON_INFRASTRUCTURES` sans le préfixe `INFRASTRUCTURE_` que l'admin utilisait pour stocker la valeur), **puis renommage** de la constante `INFRASTRUCTURE_DISPLAY_MARGIN_ON_INFRASTRUCTURES` → `INFRASTRUCTURE_DISPLAY_MARGIN_ON_TOTAL` (et clé de traduction `InfrastructureDisplayMarginOnInfrastructures` → `InfrastructureDisplayMarginOnTotal` dans les 4 fichiers de langue) pour aligner sur la sémantique métier (la cellule est rendue sur les sous-totaux et la méthode de détection est `TInfrastructure::isTotal()`). Migration des installations existantes via `sql/update.sql` (préserve la valeur stockée).
- `18.2.0` (2026-04) : Renommage de la méthode statique `TInfrastructure::isInfrastructure(&$line, $level)` en `TInfrastructure::isTotal(&$line, $level)` (`class/infrastructure.class.php`). Le nom précédent était ambigu (collision sémantique avec le nom du module) ; `isTotal` exprime clairement que la méthode détecte une ligne sous-total (`qty 91-99`). Renommage propagé sur 38 occurrences dans 8 fichiers du module ; aucun module externe utilisant la méthode (vérification `grep` sur `htdocs/`), pas de wrapper de rétrocompatibilité ajouté.
- `18.2.0` (2026-04) : Renommage des constantes `INFRASTRUCTURE_INFRASTRUCTURE_STYLE` / `INFRASTRUCTURE_INFRASTRUCTURE_BACKGROUND_COLOR` / `INFRASTRUCTURE_INFRASTRUCTURE_COLOR` → `INFRASTRUCTURE_TOTAL_STYLE` / `INFRASTRUCTURE_TOTAL_BACKGROUND_COLOR` / `INFRASTRUCTURE_TOTAL_COLOR` pour aligner sur le renommage `isInfrastructure()` → `isTotal()` de la même version (ces constantes contrôlent le rendu des lignes sous-total). Renommage parallèle des 4 clés de traduction `InfrastructureInfrastructure*` → `InfrastructureTotal*` dans les 4 fichiers de langue. Mise à jour dans `sql/data.sql`, `admin/infrastructuresetup.php`, `core/lib/infrastructure.lib.php`, `core/tpl/infrastructureline_infrastructure.tpl.php`, `core/tpl/infrastructureline_row_document.tpl.php`, `class/actions_infrastructure.class.php`. Migration des installations existantes via `sql/update.sql` (3 paires DELETE/UPDATE qui préservent la valeur).
- `18.2.0` (2026-04) : Correction d'une incohérence de nommage de constante sur l'option « Texte pour le titre de regroupement lors du passage de commandes en facture ». L'admin (`infrastructuresetup.php` ligne 200) stockait la valeur sous `INFRASTRUCTURE_TEXT_FOR_TITLE_ORDERS_TO_INVOICE`, mais le trigger (`core/triggers/interface_90_modInfrastructure_infrastructuretrigger.class.php` ligne 411, méthode `OrdersToInvoiceBloc`) lisait `INFRASTRUCTURE_TEXT_FOR_TITLE_ORDETSTOINVOICE` (typo historique « ORDETS » au lieu de « ORDERS_TO »). Conséquence : la constante saisie par l'admin n'était jamais lue, le trigger retombait sur la valeur par défaut. Alignement du trigger sur le nom utilisé par l'admin (`ORDERS_TO_INVOICE`). Aucune migration SQL nécessaire — l'admin a toujours stocké sous le bon nom.
- `18.2.0` (2026-04) : Exposition de 8 constantes auparavant utilisées dans le code mais inaccessibles depuis la page d'administration (`admin/infrastructuresetup.php`) : `INFRASTRUCTURE_FORCE_EXPLODE_ACTION_BTN` (force boutons d'action en mode éclaté hors dropdown), `INFRASTRUCTURE_STYLE_TITRES_SI_LIGNES_CACHEES` (style B/U/I des titres en mode hideblock — était déjà déclarée dans `modInfrastructure->const` mais sans UI), `INFRASTRUCTURE_TITLE_BACKGROUND_CELL_HEIGHT_OFFSET` / `INFRASTRUCTURE_TITLE_BACKGROUND_CELL_POS_Y_OFFSET` / `INFRASTRUCTURE_BACKGROUND_CELL_HEIGHT_OFFSET` / `INFRASTRUCTURE_BACKGROUND_CELL_POS_Y_OFFSET` (offsets PDF pour ajustement fin de la cellule de fond colorée des titres et sous-totaux), `INFRASTRUCTURE_DEFAULT_CHECK_SHIPPING_LIST_FOR_TITLE_DESC` (pré-coche la case « Inclure la liste des expéditions » à l'ajout d'un titre), `INFRASTRUCTURE_DISABLE_FIX_TRANSACTION` (désactive l'enrobage transactionnel SQL des actions sur blocs — zone expérimentale). Ajout des 5 constantes input à `$list['Gen']` pour persistance via le bouton update ; ajout de 13 clés de traduction par langue (libellés + tooltips help) dans les 4 fichiers `fr_FR`/`en_US`/`es_ES`/`it_IT`. La constante `INFRASTRUCTURE_DISABLE_CHECK_VERSION_MIN` reste volontairement non exposée (option de débogage interne).
- `18.2.0` (2026-04) : Refactor — extraction du rendu HTML complet du `<tr>` d'une ligne spéciale infrastructure en contexte **fiche document principale** (`printObjectLine` bloc 1, contextes `propalcard` / `ordercard` / `invoicecard` / `supplier_proposalcard` / `ordersuppliercard` / `invoicesuppliercard` / `invoicereccard`) dans un nouveau template `core/tpl/infrastructureline_row_document.tpl.php`. Le bloc 1 dans `actions_infrastructure.class.php` passe de ~220 lignes inline à un simple `include` + `return 1`. Le template prend en charge le calcul complet du colspan (multicurrency, marges, units, showprice, situation invoice…), la cellule numéro, la cellule libellé déléguée à `infrastructureline_view.tpl.php` (mode vue) ou `infrastructureline_edit.tpl.php` (mode édition), la cellule total HT avec multicurrency, la cellule edit (clone + edit / save + cancel selon mode), la cellule delete (avec delete-all sur les titres), la cellule NC checkbox, la cellule move drag-drop, la cellule selectlines, et la ligne d'extrafields des titres avec script JS de show/hide. **Troisième et dernière étape de la migration** : la méthode `printObjectLine()` est désormais un router pur, chaque branche se résume à `elseif + include + return 1`.
- `18.2.0` (2026-04) : Refactor — extraction du rendu HTML complet du `<tr>` d'une ligne spéciale infrastructure en contexte **fiche shipping (expédition existante) ou delivery (livraison)** — `printObjectLine` bloc 3 — dans un nouveau template `core/tpl/infrastructureline_row_shipping.tpl.php`. Le bloc 3 dans `actions_infrastructure.class.php` passe de ~67 lignes inline à un simple `include` + `return 1`. Le template prend en charge la cellule numéro de ligne, le libellé délégué à `infrastructureline_view.tpl.php` en contexte `'shipment'`, la cellule colspan vide, la cellule delete conditionnelle (shipping draft + `INFRASTRUCTURE_ALLOW_REMOVE_BLOCK` avec boutons « supprimer ligne » et « supprimer bloc »), et la ligne d'extrafields shipping. Deuxième étape de la migration.
- `18.2.0` (2026-04) : Refactor — extraction du rendu HTML complet du `<tr>` d'une ligne spéciale infrastructure en contexte « création d'expédition depuis une commande » (`printObjectLine` bloc 2 — contextes `commande+ordershipmentcard` ou `expeditioncard+create`) dans un nouveau template `core/tpl/infrastructureline_row_shipment.tpl.php`. Le bloc 2 dans `actions_infrastructure.class.php` passe de ~30 lignes inline à un simple `include` + `return 1`, conformément à la convention Dolibarr (cf. `core/tpl/objectline_view.tpl.php` / `objectline_edit.tpl.php`). Le template peut désormais être surchargé par un module tiers via `module_parts['tpl']`. Première étape d'une migration en 3 temps des 3 blocs de `printObjectLine` vers des templates dédiés.
- `18.2.0` (2026-04) : Application des classes CSS de niveau (`newInfrastructure` / `subtitleLevelN` / `infrastructureLevelN` / `infrastructureText`) sur les lignes spéciales aussi en contexte **shipment** (`printObjectLine` blocs 2 et 3 — création d'expédition depuis commande et fiche shipping/delivery). Auparavant la variable `$class` était calculée par `infrastructure_getLineSpecialClass($line)` mais n'était pas injectée dans le `<tr class>` de ces blocs (oversight historique) ; seul le bloc 1 l'utilisait. Les sélecteurs CSS basés sur ces classes ne s'appliquaient donc pas sur les pages d'expédition.
- `18.2.0` (2026-04) : Refactor — convergence des 3 blocs de rendu de la cellule libellé dans `printObjectLine()` (carte document principale / création d'expédition / fiche shipping-delivery) vers le même template `core/tpl/infrastructureline_view.tpl.php`. Avant : ~30 lignes inline dupliquées dans chacun des blocs 2 et 3 (rendu de l'icône `fa-tenge`, des styles `INFRASTRUCTURE_TITLE_STYLE`, du libellé selon les options `PRODUIT_DESC_IN_FORM` / `INFRASTRUCTURE_CONCAT_TITLE_LABEL_IN_INFRASTRUCTURE_LABEL`, du picto `pagebreak`). Après : 4 lignes par bloc (set `$infrastructureViewContext` + `include` du template). Le template gère désormais 2 contextes via la variable `$infrastructureViewContext` : `'document'` (défaut, comportement inchangé) et `'shipment'` (single `<td>` pour tous types y compris sous-totaux indentés sans icône, pas de boutons de pliage, `INFRASTRUCTURE_TITLE_STYLE` pour tous types, concat du titre parent sur sous-total à label vide si option activée).
- `18.2.0` (2026-04) : Refactor — extraction de la logique de calcul des classes CSS des lignes spéciales (`newInfrastructure` + `subtitleLevel{N}` / `infrastructureLevel{N}` / `infrastructureText`) dans une fonction utilitaire `infrastructure_getLineSpecialClass($line)` ajoutée à `core/lib/infrastructure.lib.php`. Cette logique était dupliquée à 4 endroits dans `actions_infrastructure.class.php` (mêmes contextes que `infrastructure_getLineSpecialStyle`). Au passage les tests numériques bruts (`$line->qty > 0 && $line->qty < 10` / `qty > 90 && qty < 100` / `qty == 50`) ont été remplacés par les helpers métier `TInfrastructure::isTitle()` / `isTotal()` / `isFreeText()` pour expliciter l'intention sémantique.
- `18.2.0` (2026-04) : Refactor — extraction de la logique de calcul du style CSS inline (`background-color` + `color`) des lignes spéciales dans une fonction utilitaire `infrastructure_getLineSpecialStyle($line)` ajoutée à `core/lib/infrastructure.lib.php`. La logique (titres `qty 1-9`, sous-totaux `qty 91-99`, calcul `colorLighten` par niveau, lecture des constantes `INFRASTRUCTURE_TITLE_BACKGROUND_COLOR` / `INFRASTRUCTURE_TITLE_COLOR` / `INFRASTRUCTURE_INFRASTRUCTURE_BACKGROUND_COLOR` / `INFRASTRUCTURE_INFRASTRUCTURE_COLOR` / `INFRASTRUCTURE_TITLE_AND_INFRASTRUCTURE_BRIGHTNESS_PERCENTAGE`) était dupliquée à 4 endroits dans `actions_infrastructure.class.php` : fiche document standard, création d'expédition depuis commande, fiche shipping/delivery et lignes d'origine via `printOriginObjectSubLine`. Centralisation en un seul appel par contexte ; ~60 lignes de PHP en moins ; toute évolution future du calcul (nuances niveaux 4-9, nouvelles options de couleur) ne nécessitera qu'une seule modification.
- `18.2.0` (2026-04) : Nouvelle valeur `'hideAll'` pour la constante `INFRASTRUCTURE_BLOC_FOLD_MODE`. En mode `hideAll`, les deux boutons de pliage sont affichés sur chaque titre : le premier (`data-toggle-all-children="0"`) garde les titres et sous-totaux enfants visibles (comportement identique à `keepTitle`) ; le second (`data-toggle-all-children="1"`) force le masquage de tout le contenu, sous-titres et sous-totaux compris. Implémentation : ajout d'un paramètre `forceHideAll` à `toggleChildFolderStatusDisplay()` (JS dans `actions_infrastructure.class.php`) qui court-circuite la logique `keepTitle` quand il vaut `true` ; le handler de clic positionne `forceHideAll = true` uniquement pour le clic du bouton 2 en mode `hideAll`. Mise à jour du dropdown admin (`infrastructuresetup.php`), de la liste des modes valides côté PHP (`['default', 'keepTitle', 'hideAll']`) et ajout des clés de traduction `InfrastructureHideAllOnFold` dans les 4 fichiers de langue.
...
TInfrastructure::isTotal(&$line, $level = -1)               // Détecte si la ligne est un sous-total
