...
*/
	...
	while($obj = $db->fetch_object($res)) {
		$resLine	= $db->query('SELECT rowid FROM  '.MAIN_DB_PREFIX.'facturedet WHERE fk_facture='.$obj->fk_facture.' ORDER BY rang ');
		$k			= 1;
		while($objLine = $db->fetch_object($resLine)) {
	...
	while($obj = $db->fetch_object($res)) {
		$resLine	= $db->query('SELECT rowid FROM  '.MAIN_DB_PREFIX.'commandedet WHERE fk_commande='.$obj->fk_commande.' ORDER BY rang ');
		$k			= 1;
		while($objLine = $db->fetch_object($resLine)) {
